...
# @jit(nopython=True)
def lbm_collision(fin, fout, equi, inlet, u, rho, mass, cell_type, omega, v, t, gravity):
    """
    ...
    for ix in range(rho.shape[0]):
        for iy in range(rho.shape[1]):
            # collision step
            collide(ix, iy, equi, fin, fout, omega, rho, mass, cell_type, v, t, gravity)
            ...
            # interface cell
                ...
                for n in range(1, 9):
                    ...
                    if cell_type[ix_next, iy_next] & CT.FLUID:
                        # streaming
                        fin[n, ix, iy] = fout[n, ix_next, iy_next]
                        mass[ix, iy] += fout[n, ix_next, iy_next]
...
@jit()
def collide(ix, iy, equi, fin, fout, omega, rho, mass, cell_type, v, t, gravity):
    """
    Perform the collision step
    """
    for n in range(9):
        fout[n, ix, iy] = 0
        fout[n, ix, iy] -= mass[ix, iy] * t[n] * -0.5
