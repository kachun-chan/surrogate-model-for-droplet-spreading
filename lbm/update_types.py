...
def update_types(rho, cell_type, mass, v, fdist, u, equi, t, rho_prev, mass_prev, u_prev, cell_type_prev):
    ...
    # set the to_interface flags
    for ix in range(rho.shape[0]):
        for iy in range(rho.shape[1]):
            if cell_type[ix, iy] & CT.GAS:
                if mass[ix, iy] > fill_offset * rho[ix, iy]:
                    cell_type[ix, iy] = CT.INTERFACE

            elif cell_type[ix, iy] & CT.FLUID:
                    if mass[ix, iy] < ( 1 - fill_offset) * rho[ix, iy]:
                        cell_type[ix, iy] = CT.INTERFACE
    ...
    for ix in range(rho.shape[0]):
        for iy in range(rho.shape[1]):
            if cell_type[ix, iy] & CT.INTERFACE:
                ...
                if mass[ix, iy] > (1 + fill_offset):
                    cell_type[ix, iy] = CT.TO_FLUID

                elif mass[ix, iy] < -fill_offset:
